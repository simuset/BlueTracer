...
In light of what just stated, BlueTracer's integration with BluePill is straightforward, since it essentially complements the \textit{Target platform simulation layer}, as it can be observed from \textit{Figure~\ref{BluePill}}.
\\
\begin{figure}[h]
\centering
...
\section{Thread Management} \label{ThreadMan}
...
\subsection{Log Files and Multithreading} \label{LogFilesMultithreading}
...
\subsection{Main Image Check} \label{MainImage}
In order to filter the logged information relative to Native APIs, BlueTracer allows the analyst to decide, through the use of a boolean parameter (\texttt{MainImage}) in the configuration file, whether only Native APIs invoked directly from the main executable of the pinned application should be traced. Therefore, as the reader can see in \textit{Figure~\ref{NTAPI Tracing}}, the first thing we do in \texttt{TraceSysCallEntry}, assuming that the \texttt{MainImage} configuration parameter has been set to \texttt{true}, is to determine if the Native API call is taking place directly from the main executable.
...
\subsection{Information Representation and Retrieval}
...
In \textit{3.2.2} it was explained how BlueTracer obtains a Native API name from its ID. After that, as shown in \textit{Figure~\ref{NTAPI Tracing}}, there must be a way to, given a specific Native API name, retrieve the corresponding \texttt{syscall\_info\_t struct}. This is done by building a hash map during the tool's initialization which maps the Native API name to the appropriate \texttt{struct}. Such idea is also adopted for named constants enumerations since a hash map is also constructed with the objective of associating the name of a named constant enumeration to the corresponding \texttt{struct}.    
...
\begin{itemize}
\item Then, if the argument's type is primitive (e.g., \texttt{int}, \texttt{bool}, etc.),  \texttt{print\_simple\_value} is invoked to record its value. In particular, the second parameter of this function is a boolean which is set to \texttt{true} if it is wanted to print the argument's value with leading zeros (e.g., if the argument is of pointer type and the address it stores is being logged ) and \texttt{false} otherwise.

\newpage
\begin{lstlisting}[xleftmargin=\dimexpr-\leftmargini,caption={\texttt{print\_arg} function},captionpos=b]
...
The way \texttt{print\_simple\_value} operates is quite straightforward:
\begin{enumerate}
...
Once the input argument values are logged, then the reference to the \texttt{syscall\_tracer struct} representing the Native API under analysis is stored in thread local data (\textit{Listing 3.1}). This is done so that, after the Native API's execution, the corresponding \texttt{syscall\_tracer} data structure can be accessed from \texttt{TraceSysCallExit}. In fact, as mentioned in Section~\ref{ThreadMan}, the \texttt{bluepill\_tls struct} associated to the running thread can be retrieved in any analysis function by means of \texttt{PIN\_GetThreadData}.
...
\section{Tracing Windows Libraries}

In Microsoft Windows, APIs are implemented as functions provided by a set of dynamically linked libraries. These, also known as DLLs, constitute Microsoft's way of implementing the concept of shared libraries in the Windows operating system. Therefore, the functions they make available can be employed by different applications \cite{DLL}. Windows APIs offer a plethora of functionalities which belong to many different categories, ranging from base services (e.g., file management) to user interface functions and network operations \cite{WinAPI}. In light of this, a Windows API has rich semantic information associated to it and tracing the sequence of APIs a malware calls provides analysts with a high-level view of the sample's behavior. This is why BlueTracer's API tracer component is undoubtedly the most important one. 
\begin{figure}[h]
\centering
\vspace{1cm}
\includegraphics[scale=0.7]{Figures/APITracing.pdf}
...
The first step taken towards APIs tracing is APIs instrumentation, i.e., placing analysis routines before and after the APIs' execution. To do this, a specific function was embedded in BluePill's image callback (the notion of image callback was previously introduced in Section~\ref{MainImage}). Such function, named \texttt{AddAnalysisRoutines}, performs the following actions for every DLL being loaded:

\begin{enumerate} 
\item It inspects the symbol table, i.e., the DLL's exports. To this end, Pin symbol objects (\texttt{SYM}s) are employed, which provide information about function symbols in applications \cite{Pin}.
\item For every symbol object, it retrieves the corresponding routine (\texttt{RTN}) object. In fact, in Pin a \texttt{RTN} represents a function usually generated by a compiler for a procedural programming language, e.g C \cite{Pin}. To retrieve the aforementioned \texttt{RTN} object, the \texttt{RTN\_FindByAddress} API was utilized, which takes as input a \texttt{RTN} memory address and returns a handle to the found \texttt{RTN}. Such memory address was determined by adding the symbol's RVA (obtained via \texttt{SYM\_Value}) to the lowest image address (fetched through \texttt{IMG\_LowAddress}).  
\item For every routine object, it retrieves the corresponding name via \texttt{RTN\_name} and checks whether the API being represented is "known", i.e., a source of information associated to the API exists.
...
\textit{drltrace} (already mentioned in Section~\ref{DBI}) comes with its own external configuration file, in which each line contains information relative to an API, as it can be observed from the sample configuration file entry on \textit{Listing 3.9}.      
\vspace{0.5cm}
\lstset{
    language=C++,
...
This API takes as input the following parameters:
\begin{itemize}
...
\item \texttt{argNumber}, a constant value containing the number of arguments for the analyzed API.
...
\item The API return address, provided by \texttt{IARG\_RETURN\_IP}. This is employed in the main image check, similarly to what explained in Section~\ref{MainImage}.
...
\subsection{Analysis before the call}
...
We believe \texttt{stackEntry}'s fields are all, once again, self-explanatory. A big role in the shadow stack management is taken by the \texttt{currentSP} field, as it will be explained in the next section. Such field stores the stack pointer's value before the API's execution, which was passed to the analysis function with the <\texttt{IARG\_REG\_VALUE}, \texttt{REG\_STACK\_PTR}> pair.

\subsection{Analysis after the call}
...
Pin has to intercept the delivery of asynchronous events from the kernel in order to keep complete control of the application.  
...
This API allows to register a notification function to execute just before the pinned application changes context due to the reception of an asynchronous event, like for instance a callback or a Windows APC. In BlueTracer, the idea is to use such registered notification function to extract and log information relative to the context change. 
\newpage
The notification function in question is of type \texttt{CONTEXT\_CHANGE\_CALLBACK} and the most important arguments it takes as input are:
\begin{itemize}
...
\subsection{Tracing User-Mode Callbacks} \label{UserModeCallbacks}
...
Right before the execution of \texttt{KiUserCallbackDispatcher}, i.e., when the registered \texttt{CONTEXT\_CALLBACK\_NOTIFICATION} function is called, the stack looks as shown in \textit{Figure~\ref{KiUserCallback}}.
\\
\begin{figure}[h]
\centering
...
\item The callback number is obtained by adding 4 (as we are on a 32-bit architecture) to the stack pointer and fetching the pointed value, in accordance to \textit{Figure~\ref{KiUserCallback}}.
...
\item We check if the address of the function falls within \texttt{User32.dll} memory address range. In fact, in \texttt{Win32} user mode callbacks are employed exclusively by \texttt{User32.dll} for windowing related aspects \cite{UCDispatcher}, meaning that the retrieved function address must belong to \texttt{User32.dll}'s memory address range. Such range is determined by retrieving a handle for \texttt{User32.dll} through \texttt{GetModuleHandle}, building an \texttt{IMG} object from it utilizing Pin's \texttt{IMG\_FindByAddress} and invoking the usual \texttt{IMG\_LowAddress} and \texttt{IMG\_HighAddress} functions. 
...
\item The callback occurrence and the callback name are recorded in the log.
\end{enumerate}
...
\subsection{Tracing Windows Asynchronous Procedure Calls}
...
\begin{enumerate}
\item The stack pointer value is retrieved, exactly in the same way as explained for user mode callbacks in Section~\ref{UserModeCallbacks}
...
The resulting log format is exactly the same as the one adopted for user mode callbacks (\textit{Listing 3.17}).
\\
\lstset{
    language=C++,
    frame=single, % draw a frame at the top and bottom of the code block
