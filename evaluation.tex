...
We \iffalse first \fi tested the tool on \textit{Al-Khaser}, a popular open-source project employed to assess how stealthy a malware analysis system is with respect to a large portion of public evasion techniques used by real malware families.
...
Finally, we tested BlueTracer on a set of benign applications, belonging to different categories, in order to assess its run-time overhead.

\iffalse
When adding JoeBox add new platform
\fi
All the tests were conducted on a virtual machine blablabla with host operating system 
